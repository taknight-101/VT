...
router.get('/teamview/', async (req, res) => {
  ...
  let currentDepartmentId = getDepartmentIdForTeamView(req);
  //AIC note:: for some reason: the above method is not deterministic, thus the fall back to the following line
  // currentDepartmentId = user.dataValues.DepartmentId

  console.log("PBS iS SOURCED HERE:: ",  currentDepartmentId)
  ...
  try {
    ...
    users_and_leaves.forEach( ul=> {

      ul.user.user_id =user.dataValues.id ;
    } )
    ...
    const user_deparment = await user.getDepartment()
    const is_junior = user_deparment.dataValues?.name.split("-")[0].toLowerCase() == "junior"
  
    console.log("ya rab" , is_junior)
    renderingContext.is_junior = is_junior ; 
    // renderingContext.user_id = user.dataValues.id;
...
const groupUsersOnTeamViewByDepartments = (usersAndLeaves) => {
  console.log("DEBUG++++++++++++++++++++++++++++++++++++++++++++++++" , usersAndLeaves)
